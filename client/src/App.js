...
function App() {
  ...
  const handleClick = async () => {
    try {
      ...
      await fetch('http://localhost:5000/api', postOptions);
      ...
      const response = await fetch('http://localhost:5000/result', getOptions);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', height: '100vh' }}>
      <div style={{ fontSize: '20px', padding: '10px', marginBottom: '10px' }}>Example Inputs:</div>
      <div style={{ fontSize: '20px', padding: '10px', marginBottom: '10px' }}>"But my God shall supply all your need according to his riches in glory by Christ Jesus."</div>
      ...
      <div style={{ display: 'flex', alignItems: 'center' }}>
        <input id="text-input" type="text" value={inputText} onChange={handleInputChange} placeholder="Type here..." style={{ fontSize: '20px', padding: '10px', borderRadius: '10px', border: '2px solid #ccc', backgroundColor: '#F5F5F5', color: 'black', boxShadow: '0px 4px 5px rgba(0, 0, 0, 0.25)', marginRight: '10px', width: '500px', height: '100px', resize: 'vertical' }} />
